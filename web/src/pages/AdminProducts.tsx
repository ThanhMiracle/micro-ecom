...
export default function AdminProducts() {
  ...
  const create = async () => {
    try {
      await productApi.post("/admin/products", { name, description, price, published: true });
      toast.success("Created & published ✨");
      setName(""); setDescription(""); setPrice(9.99);
      load();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="candy-card p-6">
        ...
        <p className="mt-1 text-sm text-slate-600">Create products & publish them to the homepage.</p>
      </div>
      ...
      <div className="candy-card p-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="md:col-span-1">
            ...
            <input className="candy-input mt-1" value={name} onChange={e => setName(e.target.value)} placeholder="Rainbow Donut" />
          </div>

          <div className="md:col-span-1">
            <div className="text-xs font-semibold text-slate-600">Price</div>
            <input className="candy-input mt-1" type="number" step="0.01" value={price} onChange={e => setPrice(Number(e.target.value))} />
          </div>

          <div className="md:col-span-3">
            <div className="text-xs font-semibold text-slate-600">Description</div>
            <textarea className="candy-input mt-1 min-h-[90px]" value={description} onChange={e => setDescription(e.target.value)}
              placeholder="Super glossy, candy-like, colorful treat…" />
          </div>
        </div>

        <button className="candy-btn mt-4 disabled:opacity-60" disabled={!name || !price} onClick={create}>
          Create & Publish
        </button>
      </div>
              ...
              <tbody>
                {rows.map((p) => (
                  <tr key={p.id} className="border-t border-white/60">
                    ...
                    <td className="py-3">
                      <span className={"rounded-full px-3 py-1 text-xs font-bold " + (p.published ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-700")}>
                        {p.published ? "Yes" : "No"}
                      </span>
                    </td>
